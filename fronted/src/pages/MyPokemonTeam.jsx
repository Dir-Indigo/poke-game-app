            ...
            <div className="mb-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-900 p-4 rounded-lg min-h-[180px]">
                    {selectedTeam.map((p, index) => (
                        <div 
                            key={p.id}
                            onClick={() => index > 0 ? handleSetLeader(p) : handleSelectForTeam(p)}
                            className={`relative bg-gray-800 p-4 rounded-lg cursor-pointer border-2 transition-all 
                                ${index === 0 
                                    ? 'border-4 border-poke-yellow-dark shadow-lg shadow-yellow-500/50' 
                                    : 'border-poke-yellow'
                                }`}
                        >
                            {index === 0 && (
                                <h2 className="absolute top-1 left-1 bg-poke-yellow-dark text-amber-400 text-xs font-bold px-2 py-1 rounded">
                                    LÍDER
                                </h2>
                            )}
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    {userPokemons
                        ...
                        .map((p) => (
                            <div 
                                key={p.id}
                                onClick={() => handleSelectForTeam(p)}
                                className="relative bg-gray-800 p-4 rounded-lg cursor-pointer border-2 border-transparent hover:border-poke-blue transition-all"
                            >
                                {p.level && (
                                    <div
                                        className="absolute top-2 right-2 w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm border-2 border-gray-900"
