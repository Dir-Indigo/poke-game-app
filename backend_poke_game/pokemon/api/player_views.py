...
class PlayerViewSet(viewsets.ModelViewSet):
    ...
    def use_heal(self, request):
        ...
        player.use_heal()
        return Response({"status": "heal used", "player": player.username, "heals_left": player.heals},
    ...
    def reset_heals(self, request):
        player = request.user
        player.reset_heals()
        return Response({'status': 'heals reset', 'player': player.username, 'heals': player.heals},
                        status=status.HTTP_200_OK)
