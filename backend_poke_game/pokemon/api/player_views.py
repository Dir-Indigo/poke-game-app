...
class PlayerSeriaizer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        # Sacamos la contraseña antes de crear el usuario
        password = validated_data.pop('password')

        # Creamos el usuario sin la contraseña (heals no cae en create_user)
        player = Player.objects.create(**validated_data)

        # Asignamos la contraseña correctamente
...
class PlayerViewSet(viewsets.ModelViewSet):
    ...
    def use_heal(self, request):
        ...
        player.use_heal()
        return Response(
            {"status": "heal used", "player": player.username, "heals_left": player.heals},
    ...
    def reset_heals(self, request):
        ...
        player.reset_heals()
        return Response(
            {'status': 'heals reset', 'player': player.username, 'heals': player.heals},
            status=status.HTTP_200_OK
        )
